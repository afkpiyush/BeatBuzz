...
app.use(session({
  secret: 'beatbuzz_secret_key_change_this_to_strong_secret',
  resave: false,
...
const db = mysql.createConnection({
  host: "localhost",
  user: "root",
  password: "Piyush@2811",
  database: "beatbuzz" // Make sure database name consistent
});
...
const transporter = nodemailer.createTransport({
  service: "gmail",
  auth: {
    user: process.env.GMAIL_USER || "rajpurohitpiyush2006@gmail.com",
    pass: (process.env.GMAIL_PASS || "iilh xuig soxs lcxt").replace(/\s+/g, "")
  }
});
...
app.post('/submit_profile', upload.single('profile_pic'), async (req, res) => {
  ...
  db.query("SELECT * FROM credentials WHERE email = ? AND is_active = 1", [email], (err, results) => {
    ...
    const sql = `INSERT INTO profiles (email, full_name, branch, clubs_part_of, domain, position, year, hometown, bio, zodiac_sign, profile_pic, profile_pic_url) 
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`;
    const values = [email, full_name, branch, clubs_part_of || '', domain || '', position || '', parseInt(year), hometown, bio, zodiac_sign, null, profile_pic_url];
...
app.get('/api/my_profile', (req, res) => {
  ...
  const sql = `
    SELECT c.username AS username, p.email, p.full_name, p.branch, p.clubs_part_of, p.domain,
           p.position, p.year, p.hometown, p.bio, p.zodiac_sign, p.profile_pic, p.profile_pic_url
    FROM credentials c
...
app.get('/api/user_profile', (req, res) => {
  ...
  db.query("SELECT email FROM credentials WHERE username = ?", [username], (err, credResults) => {
    ...
    db.query("SELECT full_name, zodiac_sign, bio, profile_pic, profile_pic_url FROM profiles WHERE email = ?", [email], (err, profileResults) => {
      if (err) {
        console.error(err);
        return res.status(500).json({ error: 'DB error' });
      }
...
app.get("/api/all_profiles", (req, res) => {
  ...
  const sql = `
    SELECT c.username AS username, p.full_name, p.zodiac_sign, p.bio, p.profile_pic, p.profile_pic_url
    FROM profiles p
...
app.get("/api/get_profile/:username", (req, res) => {
  ...
  const sql = `
    SELECT c.username AS username, p.email, p.branch, p.clubs_part_of, p.domain, p.position, p.year, p.hometown,
           p.full_name, p.profile_pic, p.profile_pic_url, p.bio, p.zodiac_sign
    FROM profiles p
    JOIN credentials c ON p.email = c.email
    WHERE c.username = ?
  `;
...
// Create a new post (image + optional caption)
app.post('/api/posts', postUpload.single('image'), async (req, res) => {
  ...
  try {
    ...
    console.error('Cloud upload (post) error:', e);
    try {
      const localDir = path.join(__dirname, 'uploads', 'posts');
      fs.mkdirSync(localDir, { recursive: true });
      const localFilename = `post-${Date.now()}-${Math.round(Math.random()*1e9)}${path.extname(req.file.originalname)}`;
      ...
      fs.writeFileSync(localPath, req.file.buffer);
      image_url = `/uploads/posts/${localFilename}`;
      console.warn('Cloud issue detected. Fell back to local storage:', image_url);
    } catch (writeErr) {
      console.error('Local upload fallback error:', writeErr);
      return res.status(500).json({ error: 'Cloud upload failed' });
    }
...
// List all posts (newest first) with author info, counts, and liked_by_me
    ...
    SELECT conv.other_username,
           ...
           (SELECT COUNT(*) FROM direct_messages dmu 
             WHERE dmu.sender_username = conv.other_username 
               ...
               AND dmu.is_read = 0) AS unread_count,
           pr.full_name, pr.profile_pic, pr.profile_pic_url
    FROM (
      SELECT 
...
app.post('/api/messages/:username', (req, res) => {
  ...
  const sql = `
    ...
    VALUES (?, ?, ?)
  `;
  db.execute(sql, [actor, other, text], (err, result) => {
    if (err) {
    ...
    const notifSql = `
      INSERT INTO notifications (username, actor, type, message)
      VALUES (?, ?, 'vibe', CONCAT(?, ' sent you a message'))
    `;
    db.execute(notifSql, [other, actor, actor], (nErr) => {
      if (nErr) {
        // Log but do not fail the message send
        console.error('Message notification insert error:', nErr);
      }
