        ...
        savePicBtn.addEventListener('click', () => {
          ...
          fetch('/api/profile/pic', { method: 'POST', credentials: 'include', body: formData })
            ...
            .then(({ ok, data }) => {
              ...
              document.querySelector('.profile-pic').src = data.profile_pic_url || '/uploads/default.jpg';
...
if (username) {
  ...
  fetch(`/api/get_profile/${encodeURIComponent(username)}`)
    ...
    .then(user => {
      ...
      document.querySelector(".profile-bio").textContent = user.bio || "No bio provided";

      const pic = document.querySelector(".profile-pic");
      pic.src = user.profile_pic_url || (user.profile_pic ? `/uploads/${user.profile_pic}` : "/uploads/default.jpg");
